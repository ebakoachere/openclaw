...
import { runListIndicators } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-list-indicators egress allowlist", () => {
  ...
  it("surfaces an aborted fetch via AbortSignal", async () => {
    ...
    controller.abort();
    await expect(runListIndicators({}, { fetchImpl }, controller.signal)).rejects.toMatchObject({
      name: "AbortError",
    });
  });
});
