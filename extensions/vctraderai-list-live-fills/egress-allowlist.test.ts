...
import { runListLiveFills } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-list-live-fills egress allowlist", () => {
  ...
  it("every captured url on the happy path matches the workspace-scoped allowlist", async () => {
    ...
    await runListLiveFills(
      { account_id: "account_id-x", cursor: "cursor-x", limit: 1 },
      { fetchImpl },
    );
