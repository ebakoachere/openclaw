// VC Trader AI BFF HTTP client (workspace-scoped REPORTS variant).
//
...
//
// REPORTS variant: carries both the reads (`GET /reports`, `GET /reports/{id}`)
// and the writes (`POST /reports`, `.../revise`, `.../retract`). The write side
// is DIRECT_CONTROL in core/openclaw/allowlist.py -- it files a report straight
// through with no staged descriptor and no human Apply -- so the server's own
// typed rejection is the only thing standing between a malformed block document
// and the workspace library. `readBffErrorDetail` therefore matters more here
// than anywhere else: the reports service writes its errors to be actionable
// ("template X is periodic, so period_key is required") and the model can only
// fix its own call on the next turn if it can READ them.
//
// The X-OpenClaw-Thread header is what makes per-specialist authorship work:
// the BFF resolves the calling sub-agent from it and stamps author_kind /
// author_key on the stored report. Without it every report is authored by the
// PM, and the per-specialist Reports tab -- the surface this feature exists for
// -- stays empty no matter how much the specialists write.
//
