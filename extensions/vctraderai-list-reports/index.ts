...
export default defineToolPlugin({
  ...
  tools: (tool) => [
    tool({
      ...
      label: "List Reports",
      description:
        "List the reports already published in this workspace's report library, newest first. Read-only: it files nothing and sends nothing. Nothing is required - call it with no arguments for the most recent page. Each entry is a card carrying the report id, template, title, subtitle, summary, who wrote it, the family/cadence/scope facets, the period covered, tags, and whether it has been delivered to the inbox yet (delivered_at). Narrow the list with template, author_kind, author_key (a specialist key - this is the per-specialist Reports tab filter), family, cadence or scope. Paging is keyset, not offset: if the response carries next_cursor, pass it back as cursor for the following page. Retracted reports are hidden unless include_archived is true. Check here BEFORE publishing a periodic report: a periodic template can only be filed once per period_key, and a duplicate is rejected outright. An empty list means nothing has been published yet, not an error.",
