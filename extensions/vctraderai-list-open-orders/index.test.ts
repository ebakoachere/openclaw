...
import plugin, { runListOpenOrders, LIST_OPEN_ORDERS_TOOL_NAME } from "./index.js";

const WORKSPACE_ID = "11111111-2222-3333-4444-555555555555";

describe("vctraderai-list-open-orders", () => {
  ...
  it("registers the list_open_orders tool with the plugin api", () => {
    const captured = createCapturedPluginRegistration({ id: "vctraderai-list-open-orders" });
  ...
  it("calls the workspace-scoped read with the owner bearer", async () => {
    ...
    expect(new URL(capturedUrl).pathname).toBe(
      `/api/v1/workspaces/${WORKSPACE_ID}/openclaw/live/orders`,
    );
