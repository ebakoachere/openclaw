...
import plugin, { LIST_PROJECTS_TOOL_NAME, runListProjects } from "./index.js";

const WORKSPACE_ID = "11111111-2222-3333-4444-555555555555";

describe("vctraderai-list-projects", () => {
  ...
  it("declares the tool name the allowlist and the persona use", () => {
    expect(LIST_PROJECTS_TOOL_NAME).toBe("list_projects");
  });

  it("refuses to run without a bound workspace, rather than calling a wrong one", async () => {
    delete process.env.PFM_WORKSPACE_ID;
    await expect(runListProjects({}, { bffFetch: async () => ({}) })).rejects.toThrow(
      /PFM_WORKSPACE_ID/,
    );
  });
  ...
  it("returns the BFF response VERBATIM rather than a projection", async () => {
    ...
    const out = await runListProjects({}, { bffFetch: async () => payload });
  ...
  it("REGISTERS the tool with the plugin api, under the contract name", () => {
    ...
    const captured = createCapturedPluginRegistration({ id: "vctraderai-list-projects" });
