...
import { runReviseReport } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-revise-report egress allowlist", () => {
  ...
  it("accepts the plugin's own workspace-scoped reports path", () => {
    expect(`/api/v1/workspaces/${WORKSPACE_ID}/reports/rep-1/revise`).toMatch(
      VCTRADERAI_BFF_ALLOWLIST_PATH_PATTERN,
    );
  });
  ...
  it("every captured url on the happy path matches the workspace-scoped allowlist", async () => {
    ...
    await runReviseReport({ report_id: "rep-1", title: "T", body: { blocks: [] } }, { fetchImpl });
