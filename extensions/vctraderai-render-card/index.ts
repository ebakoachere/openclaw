...
export default defineToolPlugin({
  ...
  tools: (tool) => [
    tool({
      ...
      label: "Render Card",
      description:
        "Render an in-chat card. The PLATFORM composes it from real data and shows it to the user; you get back one line saying what appeared, never the card itself. Use this instead of writing the card out yourself: the numbers then come from the store rather than from you, and they cannot be mistyped. THREE KINDS COMPOSE SERVER-SIDE: `chart`, `alerts` and `post_trade`. The judgement-shaped kinds (digest, watchlist, comparison, change_summary, order_ticket, signal, subagent, news) are refused by name and told to be written as a vn-artifact fence instead, because they are your account of something rather than a read. The remaining kinds (account, backtest, data, decision, econ, notebook_output, positions, risk) have no composer yet and are refused as unknown; the refusal names what IS composable, so read it rather than retrying. Required: kind. Optional: params (what the card is about — e.g. symbol and timeframe for a chart). If the data is not there you are told why, in one sentence; say that rather than describing a card the user cannot see.",
      parameters: Type.Object({
        kind: Type.Union(
          RENDER_CARD_KINDS.map((kind) => Type.Literal(kind)),
          {
            description:
              "Which card to render. Only `chart`, `alerts` and `post_trade` compose server-side. Judgement-shaped cards (digest, watchlist, comparison, change_summary, order_ticket, signal, subagent, news) are yours to write as a vn-artifact fence instead, and asking for them here returns a one-line refusal saying so. The rest are offered by this enum but have no composer yet and refuse as unknown.",
          },
        ),
        params: Type.Optional(
          Type.Object(
            {},
            {
              ...
              description:
                "What the card is about. For a chart: symbol, timeframe, and optionally bars, indicators, read, playbook, and `annotations` -- a list of marks drawn ON the chart.\n\nANNOTATIONS. Eight kinds, and NOTHING ELSE: there is no fibonacci, no vwap, no arrow. An unknown kind is dropped in silence, and so is any annotation missing a required key -- the card renders with one fewer mark and nothing tells you. A * marks a required key.\n  level     {kind, price*, tone?('r'|'s'), label?}  a horizontal price line. Without a label it is an unexplained line.\n  zone      {kind, lo*, hi*, tone?('supply'), label?}  a shaded price band; lo/hi are sorted for you.\n  trendline {kind, from*:{i,price}, to*:{i,price}, color?}  a sloped line. All four numbers required.\n  marker    {kind, i*, dir?('up'|'down'), tone?('sl'|'tp'), label?}  an arrow ON A BAR.\n  pattern   {kind, i*, price*, label*}  a named pill. An empty label counts as MISSING.\n  playbook  {kind, entryLo*, entryHi*, invalidation?, targets?:[number|{price,label}]}  a whole trade plan.\n  session   {kind, from*, to*, label?}  a shaded vertical span; from/to are BARE NUMBERS.\n  news      {kind, i*}  a vertical amber line. A label is accepted and NEVER drawn.\n\nTHE THREE THAT ARE EASY TO GET WRONG. (1) `marker` IGNORES `price` -- it attaches to its bar, above or below, not to a price level; anything that must sit at a price is a `level`. (2) `trendline` takes OBJECTS ({i, price}) while `session` takes BARE NUMBERS -- the same-looking field is spelled differently in the two, and the wrong spelling draws nothing. (3) `i` IS A BAR INDEX INTO THE SERIES YOU SENT, not a timestamp, and it is CLAMPED rather than refused: an index past the end lands on the last bar and you get a confident mark in the wrong place with no error. Emit only indexes that exist in that series, and OMIT the annotation rather than guess.\n\nAnnotate only what your tools returned. A fabricated level is a drawn claim about price.",
            },
          ),
        ),
      }),
