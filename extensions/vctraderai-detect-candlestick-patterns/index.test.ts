...
import plugin, {
  runDetectCandlestickPatterns,
  DETECT_CANDLESTICK_PATTERNS_TOOL_NAME,
} from "./index.js";

const WORKSPACE_ID = "11111111-2222-3333-4444-555555555555";

describe("vctraderai-detect-candlestick-patterns", () => {
  ...
  it("registers the detect_candlestick_patterns tool with the plugin api", () => {
  ...
  it("calls the workspace-scoped read with the owner bearer", async () => {
    ...
    await runDetectCandlestickPatterns(
      { account_id: "account_id-x", symbol: "EURUSD" },
      { fetchImpl },
    );
    expect(new URL(capturedUrl).pathname).toBe(`/api/v1/workspaces/${WORKSPACE_ID}/live/patterns`);
  ...
  it("surfaces a structured error on bff 4xx", async () => {
    ...
    await expect(
      runDetectCandlestickPatterns({ account_id: "account_id-x", symbol: "EURUSD" }, { fetchImpl }),
