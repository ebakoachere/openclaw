...
import { runListRunTrades } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-list-run-trades egress allowlist", () => {
  ...
  it("accepts this plugin own workspace-scoped path", () => {
    expect(`/api/v1/workspaces/${WORKSPACE_ID}/experiments/run-1/trade-audit`).toMatch(
      VCTRADERAI_BFF_ALLOWLIST_PATH_PATTERN,
    );
  });
  ...
  it("every captured url on the happy path matches the workspace-scoped allowlist", async () => {
    ...
    await runListRunTrades({ run_id: "run-1" }, { fetchImpl });
