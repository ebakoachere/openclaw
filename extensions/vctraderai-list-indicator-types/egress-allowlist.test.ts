...
import { runListIndicatorTypes } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-list-indicator-types egress allowlist", () => {
  ...
  it("rejects relative-traversal segments inside an allowlist-passing prefix", async () => {
    const fetchImpl = vi.fn();
    const bffFetch = createBffFetch({ fetchImpl: fetchImpl as unknown as typeof globalThis.fetch });
    await expect(bffFetch("/api/v1/openclaw/catalogue/../admin")).rejects.toBeInstanceOf(
      BffEgressViolation,
    );
    expect(fetchImpl).not.toHaveBeenCalled();
  });
  ...
  it("rejects newline / null-byte injection inside an allowlist-passing prefix", async () => {
    ...
    const bffFetch = createBffFetch({ fetchImpl: fetchImpl as unknown as typeof globalThis.fetch });
    await expect(
      bffFetch("/api/v1/openclaw/catalogue/timeframes\nGET /admin"),
    ).rejects.toBeInstanceOf(BffEgressViolation);
    await expect(bffFetch("/api/v1/openclaw/catalogue/timeframes\0/admin")).rejects.toBeInstanceOf(
      BffEgressViolation,
    );
    expect(fetchImpl).not.toHaveBeenCalled();
  });
  ...
  it("rejects an absolute external URL with a valid-looking workspace prefix", async () => {
    ...
    const bffFetch = createBffFetch({ fetchImpl: fetchImpl as unknown as typeof globalThis.fetch });
    await expect(
      bffFetch("https://evil.example.com/api/v1/openclaw/catalogue/timeframes"),
    ).rejects.toBeInstanceOf(BffEgressViolation);
    expect(fetchImpl).not.toHaveBeenCalled();
  });
  ...
  it("surfaces an aborted fetch via AbortSignal", async () => {
    ...
    controller.abort();
    await expect(runListIndicatorTypes({ fetchImpl }, controller.signal)).rejects.toMatchObject({
      name: "AbortError",
    });
  });
});
