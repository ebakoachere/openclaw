...
export default defineToolPlugin({
  ...
  description:
    "Autonomously cancel a live working order while the account is set to act autonomously. CANCELLING A WORKING ORDER IS DE-RISKING, so it does NOT require the account to be set to act autonomously -- the execution mode gates entries, not exits, and this runs in Manual mode too. IT CAN STILL BE REFUSED by conditions that are not the mode: a durable trading HALT, a closed live-execution gate, or agent consent revoked for the account. A refusal returns execution_status 'refused' with a lock_reason, downgraded_to_staged false, and NOTHING is staged -- there is no card and no queue entry, so relay the lock_reason as written and tell the owner they can cancel it themselves. A cancel that does not go through leaves the order LIVE and still able to fill.",
  tools: (tool) => [
    tool({
