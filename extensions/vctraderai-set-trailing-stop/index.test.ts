...
import plugin, { SET_TRAILING_STOP_TOOL_NAME, runSetTrailingStop } from "./index.js";

const WORKSPACE_ID = "11111111-2222-3333-4444-555555555555";

describe("vctraderai-set-trailing-stop", () => {
  ...
  it("declares the tool name the allowlist and the persona use", () => {
    expect(SET_TRAILING_STOP_TOOL_NAME).toBe("set_trailing_stop");
  });
  ...
  it("refuses to run without a bound workspace, rather than calling a wrong one", async () => {
    ...
    await expect(
      runSetTrailingStop(
        { account_id: "acct-1", position_id: "pos-9", distance: "15", units: "RELATIVE_PIPS" },
        { bffFetch: async () => ({}) },
      ),
  ...
  it("puts EVERY required field on the wire body", async () => {
    ...
    for (const key of ["account_id", "distance", "units"]) {
  ...
  it("returns the BFF response VERBATIM rather than a projection", async () => {
    ...
    const out = await runSetTrailingStop(
      { account_id: "acct-1", position_id: "pos-9", distance: "15", units: "RELATIVE_PIPS" },
  ...
  it("REGISTERS the tool with the plugin api, under the contract name", () => {
    ...
    const captured = createCapturedPluginRegistration({ id: "vctraderai-set-trailing-stop" });
