...
import plugin, { GET_RUN_EQUITY_CURVE_TOOL_NAME, runGetRunEquityCurve } from "./index.js";

const WORKSPACE_ID = "11111111-2222-3333-4444-555555555555";

describe("vctraderai-get-run-equity-curve", () => {
  ...
  it("declares the tool name the allowlist and the persona use", () => {
    expect(GET_RUN_EQUITY_CURVE_TOOL_NAME).toBe("get_run_equity_curve");
  });

  it("refuses to run without a bound workspace, rather than calling a wrong one", async () => {
    delete process.env.PFM_WORKSPACE_ID;
    await expect(
      runGetRunEquityCurve({ run_id: "run-1" }, { bffFetch: async () => ({}) }),
  ...
  it("returns the BFF response VERBATIM rather than a projection", async () => {
    ...
    const out = await runGetRunEquityCurve({ run_id: "run-1" }, { bffFetch: async () => payload });
  ...
  it("REGISTERS the tool with the plugin api, under the contract name", () => {
    ...
    const captured = createCapturedPluginRegistration({ id: "vctraderai-get-run-equity-curve" });
