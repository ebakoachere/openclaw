...
import { runSearchDataVault } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-search-data-vault egress allowlist", () => {
  ...
  it("every captured url on the happy path matches the workspace-scoped allowlist", async () => {
    ...
    await runSearchDataVault({ query: "atr", limit: 3 }, { fetchImpl });
  ...
  it("rejects the propose chokepoint - this tool is a read and must never stage", async () => {
    const fetchImpl = vi.fn();
    const bffFetch = createBffFetch({ fetchImpl: fetchImpl as unknown as typeof globalThis.fetch });
    await expect(bffFetch("/api/v1/openclaw/stage")).rejects.toBeInstanceOf(BffEgressViolation);
    expect(fetchImpl).not.toHaveBeenCalled();
  });
