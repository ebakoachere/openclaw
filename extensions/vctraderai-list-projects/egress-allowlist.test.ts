...
import { runListProjects } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-list-projects egress allowlist", () => {
  ...
  it("accepts this plugin own workspace-scoped path", () => {
    expect(`/api/v1/workspaces/${WORKSPACE_ID}/projects`).toMatch(
      VCTRADERAI_BFF_ALLOWLIST_PATH_PATTERN,
    );
  });
  ...
  it("every captured url on the happy path matches the workspace-scoped allowlist", async () => {
    ...
    await runListProjects({}, { fetchImpl });
