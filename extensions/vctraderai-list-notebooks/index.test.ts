...
import plugin, { runListNotebooks, LIST_NOTEBOOKS_TOOL_NAME } from "./index.js";

const WORKSPACE_ID = "11111111-2222-3333-4444-555555555555";

describe("vctraderai-list-notebooks", () => {
  ...
  it("registers the list_notebooks tool with the plugin api", () => {
    const captured = createCapturedPluginRegistration({ id: "vctraderai-list-notebooks" });
  ...
  it("calls the workspace-scoped read with the owner bearer", async () => {
    ...
    await runListNotebooks(
      { project_id: "aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee", include_archived: true, limit: 5 },
      { fetchImpl },
    );
    expect(new URL(capturedUrl).pathname).toBe(
      `/api/v1/workspaces/${WORKSPACE_ID}/openclaw/research/notebooks`,
    );
