...
export default defineToolPlugin({
  ...
  tools: (tool) => [
    tool({
      ...
      label: "Lint Strategy",
      description:
        "Lint vbt/Shape-A strategy Python against the six-key runtime contract: run(data, params, context) must return { long_entries, short_entries, long_exits, short_exits, sl_stop, tp_stop } with positive-finite sl_stop/tp_stop, using only the allowed imports/helpers. Returns { passed, errors: [{ code, message, fix_hint }] }; passed=false is a normal result, not a call failure. Use it on a vbt run(...) source before create_strategy or update_strategy. It runs ONLY the six-key contract: a valid runtime_tag='nautilus' class is falsely rejected here with AST_GUARD_REJECTION denied_import errors whose fix_hint says to delete the nautilus_trader imports - do not lint a Nautilus class here and do not follow those hints. Before create_strategy or update_strategy, ensure its StrategyConfig declares instrument_id, pfm_initial_cash, and pfm_risk_fraction and reads both risk fields when sizing; those tools validate the class-native contract themselves. Persists nothing.",
