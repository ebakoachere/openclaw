...
import { runListSessions } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-list-sessions egress allowlist", () => {
  ...
  it("surfaces an aborted fetch via AbortSignal", async () => {
    ...
    controller.abort();
    await expect(runListSessions({ fetchImpl }, controller.signal)).rejects.toMatchObject({
      name: "AbortError",
    });
  });
});
