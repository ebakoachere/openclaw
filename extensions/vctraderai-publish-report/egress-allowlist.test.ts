...
import { runPublishReport } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-publish-report egress allowlist", () => {
  ...
  it("accepts the plugin's own workspace-scoped reports path", () => {
    expect(`/api/v1/workspaces/${WORKSPACE_ID}/reports`).toMatch(
      VCTRADERAI_BFF_ALLOWLIST_PATH_PATTERN,
    );
  });
  ...
  it("every captured url on the happy path matches the workspace-scoped allowlist", async () => {
    ...
    await runPublishReport(
      { template: "research_memo", title: "T", body: { blocks: [] } },
      { fetchImpl },
    );
