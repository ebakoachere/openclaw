...
import { runGetRiskManagerSource } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-get-risk-manager-source egress allowlist", () => {
  ...
  it("surfaces an aborted fetch via AbortSignal", async () => {
    ...
    controller.abort();
    await expect(
      runGetRiskManagerSource({ risk_manager_id: "rm-1" }, { fetchImpl }, controller.signal),
