...
import { runEconCalendarTail } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-econ-calendar-tail egress allowlist", () => {
  it("every captured url on the happy path matches the allowlist", async () => {
    ...
    await runEconCalendarTail({ n: 5 }, { fetchImpl });
  ...
  it("rejects newline / null-byte injection inside an allowlist-passing prefix", async () => {
    ...
    const bffFetch = createBffFetch({ fetchImpl: fetchImpl as unknown as typeof globalThis.fetch });
    await expect(
      bffFetch("/api/v1/openclaw/data/econ-calendar/tail\nGET /admin"),
    ).rejects.toBeInstanceOf(BffEgressViolation);
    await expect(
      bffFetch("/api/v1/openclaw/data/econ-calendar/tail\0/admin"),
    ).rejects.toBeInstanceOf(BffEgressViolation);
    expect(fetchImpl).not.toHaveBeenCalled();
  });
  ...
  it("rejects an absolute external URL with a valid-looking workspace prefix", async () => {
    ...
    const bffFetch = createBffFetch({ fetchImpl: fetchImpl as unknown as typeof globalThis.fetch });
    await expect(
      bffFetch("https://evil.example.com/api/v1/openclaw/data/econ-calendar/tail"),
    ).rejects.toBeInstanceOf(BffEgressViolation);
    expect(fetchImpl).not.toHaveBeenCalled();
  });
  ...
  it("surfaces an aborted fetch via AbortSignal", async () => {
    ...
    controller.abort();
    await expect(
      runEconCalendarTail({ n: 5 }, { fetchImpl }, controller.signal),
