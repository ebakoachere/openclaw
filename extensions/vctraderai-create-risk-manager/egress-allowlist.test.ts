...
import { runCreateRiskManager } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-create-risk-manager egress allowlist", () => {
  ...
  it("every captured url on the happy path matches the allowlist", async () => {
    ...
    await runCreateRiskManager({ name: "RM" }, { fetchImpl });
  ...
  it("surfaces an aborted fetch via AbortSignal", async () => {
    ...
    controller.abort();
    await expect(
      runCreateRiskManager({ name: "RM" }, { fetchImpl }, controller.signal),
