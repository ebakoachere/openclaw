  ...
  return {
    ...
    retrySuggestion:
      typeof suggestion === "string" && suggestion.length > 0
        ...
        : undefined,
    // Same envelope record as `code` / `message` / `retry_suggestion`: the BFF
    // builds `{"detail": {"error": {code, message, trace_id, retry_suggestion,
    // problems, error_codes}}}`, so these need no separate lookup path.
...
export class BffRequestError extends Error {
  ...
  constructor(detail: BffError) {
    ...
    const head = detail.retrySuggestion
      ...
      : summary;
    // The suggestion says "read problems[].fix_hint". This is what makes that
    // sentence true.
