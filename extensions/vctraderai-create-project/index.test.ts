...
import plugin, { CREATE_PROJECT_TOOL_NAME, runCreateProject } from "./index.js";

const WORKSPACE_ID = "11111111-2222-3333-4444-555555555555";

describe("vctraderai-create-project", () => {
  ...
  it("refuses to run without a bound workspace, rather than calling a wrong one", async () => {
    delete process.env.PFM_WORKSPACE_ID;
    await expect(
      runCreateProject({ name: "Gold scalping research" }, { bffFetch: async () => ({}) }),
  ...
  it("returns the BFF response VERBATIM rather than a projection", async () => {
    ...
    const out = await runCreateProject(
      { name: "Gold scalping research" },
  ...
  it("REGISTERS the tool with the plugin api, under the contract name", () => {
    ...
    const captured = createCapturedPluginRegistration({ id: "vctraderai-create-project" });
