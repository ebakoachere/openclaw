...
import { runMoveArtifactToProject } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-move-artifact-to-project egress allowlist", () => {
  ...
  it("accepts this plugin own workspace-scoped path", () => {
    expect(`/api/v1/workspaces/${WORKSPACE_ID}/experiments/run-7/move`).toMatch(
      VCTRADERAI_BFF_ALLOWLIST_PATH_PATTERN,
    );
  });
