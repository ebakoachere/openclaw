...
import { runCreateIndicator } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-create-indicator egress allowlist", () => {
  ...
  it("every captured url on the happy path matches the allowlist", async () => {
    ...
    await runCreateIndicator({ intent_brief: "x" }, { fetchImpl });
  ...
  it("surfaces an aborted fetch via AbortSignal", async () => {
    ...
    controller.abort();
    await expect(
      runCreateIndicator({ intent_brief: "x" }, { fetchImpl }, controller.signal),
