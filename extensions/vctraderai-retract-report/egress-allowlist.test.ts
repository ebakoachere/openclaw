...
import { runRetractReport } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-retract-report egress allowlist", () => {
  ...
  it("accepts the plugin's own workspace-scoped reports path", () => {
    expect(`/api/v1/workspaces/${WORKSPACE_ID}/reports/rep-1/retract`).toMatch(
      VCTRADERAI_BFF_ALLOWLIST_PATH_PATTERN,
    );
  });
  ...
  it("every captured url on the happy path matches the workspace-scoped allowlist", async () => {
    ...
    await runRetractReport({ report_id: "rep-1", reason: "double-counted fills" }, { fetchImpl });
