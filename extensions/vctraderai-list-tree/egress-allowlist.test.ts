...
import { runListTree } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-list-tree egress allowlist", () => {
  ...
  it("rejects newline / null-byte injection inside an allowlist-passing prefix", async () => {
    const fetchImpl = vi.fn();
    const bffFetch = createBffFetch({ fetchImpl: fetchImpl as unknown as typeof globalThis.fetch });
    await expect(bffFetch("/api/v1/openclaw/data/tree\nGET /admin")).rejects.toBeInstanceOf(
      BffEgressViolation,
    );
    await expect(bffFetch("/api/v1/openclaw/data/tree\0/admin")).rejects.toBeInstanceOf(
      BffEgressViolation,
    );
    expect(fetchImpl).not.toHaveBeenCalled();
  });
  ...
  it("rejects an absolute external URL with a valid-looking workspace prefix", async () => {
    ...
    const bffFetch = createBffFetch({ fetchImpl: fetchImpl as unknown as typeof globalThis.fetch });
    await expect(
      bffFetch("https://evil.example.com/api/v1/openclaw/data/tree"),
  ...
  it("surfaces an aborted fetch via AbortSignal", async () => {
    ...
    controller.abort();
    await expect(runListTree({}, { fetchImpl }, controller.signal)).rejects.toMatchObject({
      name: "AbortError",
    });
  });
});
