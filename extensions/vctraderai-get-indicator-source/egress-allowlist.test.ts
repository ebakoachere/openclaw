...
import { runGetIndicatorSource } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-get-indicator-source egress allowlist", () => {
  ...
  it("surfaces an aborted fetch via AbortSignal", async () => {
    ...
    controller.abort();
    await expect(
      runGetIndicatorSource({ indicator_id: "ind-1" }, { fetchImpl }, controller.signal),
