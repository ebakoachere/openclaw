...
import { runGetRunEquityCurve } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-get-run-equity-curve egress allowlist", () => {
  ...
  it("accepts this plugin own workspace-scoped path", () => {
    expect(`/api/v1/workspaces/${WORKSPACE_ID}/experiments/run-1/run-studio`).toMatch(
      VCTRADERAI_BFF_ALLOWLIST_PATH_PATTERN,
    );
  });
  ...
  it("every captured url on the happy path matches the workspace-scoped allowlist", async () => {
    ...
    await runGetRunEquityCurve({ run_id: "run-1" }, { fetchImpl });
