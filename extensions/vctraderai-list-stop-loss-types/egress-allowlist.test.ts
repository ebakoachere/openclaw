...
import { runListStopLossTypes } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-list-stop-loss-types egress allowlist", () => {
  ...
  it("surfaces an aborted fetch via AbortSignal", async () => {
    ...
    controller.abort();
    await expect(runListStopLossTypes({ fetchImpl }, controller.signal)).rejects.toMatchObject({
      name: "AbortError",
    });
  });
});
