...
import plugin, {
  runListLiveAccountsForDeployment,
  LIST_LIVE_ACCOUNTS_FOR_DEPLOYMENT_TOOL_NAME,
} from "./index.js";

const WORKSPACE_ID = "11111111-2222-3333-4444-555555555555";

describe("vctraderai-list-live-accounts-for-deployment", () => {
  ...
  it("registers the list_live_accounts_for_deployment tool with the plugin api", () => {
  ...
  it("calls the workspace-scoped read with the owner bearer", async () => {
    ...
    await runListLiveAccountsForDeployment({}, { fetchImpl });
    expect(new URL(capturedUrl).pathname).toBe(
      `/api/v1/workspaces/${WORKSPACE_ID}/openclaw/deployment/live-accounts`,
    );
  ...
  it("surfaces a structured error on bff 4xx", async () => {
    ...
    await expect(runListLiveAccountsForDeployment({}, { fetchImpl })).rejects.toMatchObject({
