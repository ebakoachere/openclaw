...
import { runListRunFolds } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-list-run-folds egress allowlist", () => {
  ...
  it("accepts this plugin own workspace-scoped path", () => {
    expect(`/api/v1/workspaces/${WORKSPACE_ID}/walkforward/run-1/fold-summaries`).toMatch(
      VCTRADERAI_BFF_ALLOWLIST_PATH_PATTERN,
    );
  });
  ...
  it("every captured url on the happy path matches the workspace-scoped allowlist", async () => {
    ...
    await runListRunFolds({ run_id: "run-1" }, { fetchImpl });
