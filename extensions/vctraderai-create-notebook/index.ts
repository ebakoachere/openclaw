...
export type CreateNotebookParams = {
  ...
  /** Template branch only: the run recorded + snapshotted. Ignored when you author cells. */
  run_id?: string;
...
const NOTEBOOK_FIELD_DESCRIPTION = [
  ...
  'OTHER GOVERNED READS. Run-scoped series, run id passed as a LITERAL string (no run_id name is bound): vctrader.list_run_trades("<run id>"), vctrader.get_run_equity_curve("<run id>"), vctrader.list_experiment_runs() - owner-scoped, no account needed. ACCOUNT-SCOPED perception: get_multi_timeframe_candles, get_atr, classify_regime and detect_support_resistance all require a linked broker account_id and, in a workspace with none, return an in-band error dict such as {"error": "get_multi_timeframe_candles requires an account_id."}. They are the wrong tool for a plain market-data question.',
      ...
      parameters: Type.Object(
        {
          ...
          run_id: Type.Optional(
            Type.String({
              description:
                'The backtest/experiment run to analyse; the value comes from list_experiment_runs -> rows[].run_id. Honoured ONLY when you pass no cells (template branch): the generated papermill parameters cell records it and the backend embeds that run\'s equity/trades/metrics snapshot at create. Omit it and the template runs green but analyses no run (SEAM_STATUS "no_run_reference", embed {"available": false}). If you DO author cells it is recorded NOWHERE -- no snapshot, no parameters cell, papermill params {} -- so `run_id` is unbound in the kernel and vctrader.list_run_trades(run_id) raises NameError. Write the run id as a literal string in your cells.',
