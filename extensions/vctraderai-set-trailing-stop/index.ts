...
export default defineToolPlugin({
  ...
  description:
    "Ask the broker to attach a server-side trailing stop to a live position. A success means the broker ACCEPTED the request, never that the trail is active: server-side trailing is broker-dependent and is not proven on the live servers. Blocks during a durable halt, because attaching a trail is not de-risking. If the account is NOT set to act autonomously the call does NOT go through and NOTHING is staged for approval: it returns downgraded_to_staged true with a lock_reason, and there is no card, no queue entry and no pending approval anywhere. Tell the owner the account is not currently set to act autonomously, name the lock_reason, and ask them to place it themselves or change the account's execution mode. Never say the action is staged or awaiting approval.",
  tools: (tool) => [
    tool({
