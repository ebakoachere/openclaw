...
export default defineToolPlugin({
  ...
  description:
    "Autonomously modify a live position's stop-loss / take-profit while the account is set to act autonomously. If it is not, the call does NOT go through and NOTHING is staged for approval: it returns execution_status 'refused' with a lock_reason and downgraded_to_staged false. Adjusting protection is NOT de-risking -- a stop can be widened -- so unlike a close it is still gated by the execution mode. There is no card, no queue entry and no pending approval anywhere; tell the owner the account is not currently set to act autonomously, name the lock_reason, and ask them to do it themselves or change the account's execution mode. Never say the action is staged, pending approval or awaiting a card. A protection change that does not go through leaves the position on its OLD stop, or with none.",
  tools: (tool) => [
    tool({
