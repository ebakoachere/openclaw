...
import { runGetOrderOutcome } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-get-order-outcome egress allowlist", () => {
  ...
  it("every captured url on the happy path matches the workspace-scoped allowlist", async () => {
    ...
    await runGetOrderOutcome({ client_order_id: "client-1" }, { fetchImpl });
