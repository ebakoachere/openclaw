// VC Trader AI BFF HTTP client (market-context-frame variant).
//
...
//
// X-OpenClaw-Workspace IS STILL STAMPED, even though this variant is read-only.
// The market-frames recall route does NOT depend on `require_specialist_authority`,
// so it does not need the header to authorise. It is kept anyway because the
// write-side sibling DOES need it, and a future plugin cloned from THIS copy
// would otherwise silently lose it: `vctraderai-workspace-header-sweep.test.ts`
// only guards the gateway-token cluster, so it would not catch the regression.
// Dropping it here would save nothing and arm a later defect.
//
...
export type BffFetchOptions = {
   ...
   * defaults to POST (unless `method` is set explicitly). This variant is used
   * GET-only -- market-frame recall is a read -- but the capability is kept so
   * the helper stays byte-comparable with its siblings.
   */
  body?: unknown;
  signal?: AbortSignal;
  /**
