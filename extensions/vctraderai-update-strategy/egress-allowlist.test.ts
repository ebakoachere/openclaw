...
import { runUpdateStrategy } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-update-strategy egress allowlist", () => {
  ...
  it("every captured url on the happy path matches the allowlist", async () => {
    ...
    await runUpdateStrategy({ strategy_id: "str-9" }, { fetchImpl });
  ...
  it("surfaces an aborted fetch via AbortSignal", async () => {
    ...
    controller.abort();
    await expect(
      runUpdateStrategy({ strategy_id: "str-9" }, { fetchImpl }, controller.signal),
